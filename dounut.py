...
class Toroid:
	def __init__(self, R1, R2):
		...
		while phi <= 360:
			...
			for point in circle_points:
				self.points.append(tuple(np.matmul(point,Y_ROTATION(phi))))

			for point in normal_points:
				self.surface_normals.append(tuple(np.matmul(point,Y_ROTATION(phi))))
	...
	def update_y(self, theta_y):
		...
		for i in range(len(self.points)):
			point = self.points[i]
			self.points[i] = tuple(np.matmul(point,Y_ROTATION(theta_y)))

		for i in range(len(self.surface_normals)):
			point = self.surface_normals[i]
			self.surface_normals[i] = tuple(np.matmul(point,Y_ROTATION(theta_y)))

	def update_x(self, theta_x):

		for i in range(len(self.points)):
			point = self.points[i]
			self.points[i] = tuple(np.matmul(point,X_ROTATION(theta_x)))

		for i in range(len(self.surface_normals)):
			point = self.surface_normals[i]
			self.surface_normals[i] = tuple(np.matmul(point,X_ROTATION(theta_x)))
